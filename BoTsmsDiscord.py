...
token = "OTY3MDY3MDA3ODg5NTcxOTAy.YmK5VQ.5jIlVBigCf0UO87AwxOBYtbNefg"
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 2001):
      ...
      embes = discord.Embed(title="𝙎𝙥𝙖𝙢𝙎𝙈𝙎 𝙁𝙧𝙚𝙚 ッ", description="Bot by : Donut",color=0xff4612)
      embes.add_field(name="กำลังยิงไปที่เบอร์",value=phone)
      ima = "https://share.creavite.co/2myPEq4noufQLFoq.gif"
      ...
      BBot(phone,amount)
    	...
    	await ctx.channel.send(" ยิงไม่เกิน 2000 ")
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title="Bot BY: Donut",description="วิธิใช้",color=0xff4612)
	emBed.add_field(name="คำสั่ง",value="+sms [เบอร์] [จำนวน]")
	emBed.add_field(name="Discord",value="https://discord.gg/GhtCFwzAd5")
